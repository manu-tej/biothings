...
const WorkflowCard = memo(
  ({ workflow, onStatusUpdate, onViewDetails }: WorkflowCardProps) => {
...
export default function WorkflowsPage() {
  ...
  const updateWorkflowStatus = useMutation({
    mutationFn: async ({ workflowId, status }: { workflowId: string; status: string }) => {
      const response = await fetch(
        `${apiClient['apiBaseUrl']}/api/workflows/${workflowId}/status`,
        {
          method: 'PUT',
  ...
  const handleQuickAction = useCallback(
    (action: 'drug_discovery' | 'production_scaleup' | 'quality_control') => {
