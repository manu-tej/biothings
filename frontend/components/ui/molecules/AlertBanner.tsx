    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <h3 className={clsx(sizes.title, styles.title)}>{title}</h3>

          {message && <p className={clsx('mt-1', sizes.message, styles.message)}>{message}</p>}
        ...
        {dismissible && (
          <div className="flex-shrink-0">
            <Button
              ...
              icon={<X />}
              className={clsx('hover:bg-black/5 dark:hover:bg-white/5', styles.icon)}
...
export const ToastAlert = forwardRef<HTMLDivElement, ToastAlertProps>(
  (
    { position = 'top-right', duration = 5000, onClose, dismissible = true, ...alertProps },
    ref
  ) => {
    const [isVisible, setIsVisible] = useState(true)
