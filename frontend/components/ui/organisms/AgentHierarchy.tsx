...
export const AgentHierarchy = forwardRef<HTMLDivElement, AgentHierarchyProps>(
  (
    ...
    const renderAgentNode = (node: HierarchyNode) => {
      ...
      return (
        <div key={node.id} style={{ paddingLeft: `${node.level * 20}px` }} className="py-1">
          <AgentCard
            id={node.id}
            name={node.name}
            {...node.agent}
    ...
    return (
      <Card
        ...
        variant="default"
        padding="none"
        className={clsx('flex flex-col', isFullscreen && 'fixed inset-0 z-50', className)}
        testId={testId}
      >
        {/* Header */}
        ...
        {/* Content */}
        <div className="flex-1 overflow-auto" style={{ maxHeight: containerHeight }}>
          {filteredNodes.length === 0 ? (
            <div className="flex items-center justify-center h-32">
              <p className="text-gray-500 dark:text-gray-400">No agents or groups found</p>
            </div>
          ) : (
            <div className="p-2 space-y-1">{filteredNodes.map(renderNode)}</div>
          )}
        </div>
