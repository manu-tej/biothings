...
import { Home, BarChart3, Users, Workflow, Settings, ChevronLeft, Activity } from 'lucide-react'
...
export const Sidebar: React.FC<SidebarProps> = ({ className }) => {
  ...
  const runningWorkflows = Array.from(workflows.values()).filter(
    (w) => w.status === 'running'
  ).length
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
        {!isCollapsed && (
          <div className="flex items-center space-x-2">
            ...
            <span className="font-bold text-lg text-gray-900 dark:text-white">BioThings</span>
          </div>
        )}
        <Button
          variant="ghost"
            ...
            >
              ...
              {!isCollapsed && (
                <>
                  ...
                  {item.badge && (
                    <Badge variant={item.badgeVariant || 'default'} size="xs" className="ml-auto">
                      {item.badge}
                    </Badge>
                  )}
                </>
