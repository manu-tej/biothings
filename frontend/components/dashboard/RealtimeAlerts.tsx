...
import { AlertCircle, AlertTriangle, Info, CheckCircle, X, Bell, BellOff } from 'lucide-react'
...
const AlertItem = React.memo(
  ({ alert, onDismiss }: AlertItemProps) => {
    ...
    `}
      >
        <div className="flex items-start justify-between">
          <div className="flex items-start space-x-3 flex-1">
            ...
            <div className="flex-1">
              <p className={`text-sm font-medium ${config.textColor}`}>{alert.type}</p>
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">{timeAgo}</p>
      </div>
    )
  },
  (prevProps, nextProps) => {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg p-6 border border-gray-200 dark:border-gray-700 h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto space-y-3">
        {alerts.length > 0 ? (
          alerts.map((alert) => <AlertItem key={alert.id} alert={alert} onDismiss={dismissAlert} />)
        ) : (
