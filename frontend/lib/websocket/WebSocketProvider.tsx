...
export function useMultipleConnections(configs: Array<{ id: string; config: WebSocketConfig }>) {
  ...
  const connectAll = React.useCallback(async () => {
    ...
    const results = await Promise.allSettled(configs.map(({ id, config }) => connect(id, config)))
