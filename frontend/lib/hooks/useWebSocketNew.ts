...
export function useWebSocket(options: UseWebSocketOptions = {}) {
  ...
  const { status, connect, disconnect, send, subscribe, getConnectionStatus } = useWebSocketContext();
  ...
  const convertToLegacyMessage = useCallback((message: WebSocketMessage): LegacyWebSocketMessage => {
    return {
  ...
  const sendMessage = useCallback((message: LegacyWebSocketMessage) => {
