    ...
    (set, get) => ({
      ...
      send: (topic, data, connectionId) => {
        ...
        if (connectionId) {
          ...
          const activeConnection = Array.from(state.connections.values()).find(
      ...
      broadcast: (topic, data) => {
        const state = get()
        const activeConnections = Array.from(state.connections.values()).filter(
          (c) => c.status === 'connected'
        )
      ...
      getActiveConnections: () => {
        return Array.from(get().connections.values()).filter((c) => c.status === 'connected')
      },
