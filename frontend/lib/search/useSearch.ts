...
export function useSearch(options: UseSearchOptions = {}): UseSearchReturn {
  ...
  useEffect(() => {
    const searchableItems = [
      ...
      ...Array.from(workflows.values()).map(workflow => SearchEngine.workflowToSearchable(workflow)),
    ];
  ...
  const saveQuery = useCallback((name: string) => {
    if (query && name) {
  ...
  const executeSavedQuery = useCallback((id: string) => {
